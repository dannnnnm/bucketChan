...
<template>
    <a href="" @click.prevent="dialog = true">No. {{ shownId }}</a>
        ...
        <v-card title="Responder a un hilo">
            <v-container fluid>
                <v-form @submit.prevent="createResponse()" v-model="dataValid">
                    ...
                    <v-row justify="center">
                        <v-col cols="12" :sm="4"> Image: </v-col>
                        <v-col cols="12" :sm="8">
                    ...
                    <v-row justify="center">
                        <v-btn color="blue" type="submit" :disabled="!dataValid"> Submit </v-btn>
                    </v-row>
                </v-form>
            </v-container>
        </v-card>
...
export default {
    data() {
        return {
            ...
            dialog: false,
            rules: {
                lenMin: (lenMin: number) => (v: string | any[]) =>
                    (v || '').length >= lenMin || `Mínimo ${lenMin} caracteres`
            }
        }
    },
    props: {
    ...
    methods: {
        ...
        createResponse() {
            ...
            axios
                .post(
                    `http://${window.location.hostname}:3000/board/${this.responseData.boardShortName}/${this.threadId}/answer`,
                    form,
                    {
                        headers: {
